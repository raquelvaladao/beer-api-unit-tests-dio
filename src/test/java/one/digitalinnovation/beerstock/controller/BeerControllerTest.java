...
public class BeerControllerTest {
    ...
    //Setup do mockMvc do próprio Spring pra SIMULAR retorno de uma API (JSON)
    @BeforeEach
    void setUp() {
    ...
    void whenPOSTIsCalledThenABeerIsCreated() throws Exception {
        // given -> vamos testar o método do @PostMapping, quando ele é chamado, uma cerveja
        //é criada. o beerDTO será passado via parâmetro lá no Controller,
        //então devemos esperar uma criação.
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

        // when -> o método retorna DTO.
        when(beerService.createBeer(beerDTO)).thenReturn(beerDTO);

        // then -> chamamos a camada de controller, convertemos um objeto em JSON para
    ...
    @Test
    //vamos validar a obrigatoriedade de um campo
    void whenPOSTIsCalledWithoutRequiredFieldThenAnErrorIsReturned() throws Exception {
        // given
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();
        beerDTO.setBrand(null);
        //then -> não temos  mock do service pra create(), visto que não chegamos a executar
        //esse método, já que o campo obrigatório está null.
    ...
    void whenGETIsCalledWithValidNameThenOkStatusIsReturned() throws Exception {
        // given -> DTO do controller
        BeerDTO beerDTO = BeerDTOBuilder.builder().build().toBeerDTO();

        //when -> encontramos um objeto
        when(beerService.findByName(beerDTO.getName())).thenReturn(beerDTO);

        // then -> Ok Status
        mockMvc.perform(MockMvcRequestBuilders.get(BEER_API_URL_PATH + "/" + beerDTO.getName())
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk()) //Matcher com status HTTP.OK
                .andExpect(jsonPath("$.name", is(beerDTO.getName())))
                .andExpect(jsonPath("$.brand", is(beerDTO.getBrand())))
                .andExpect(jsonPath("$.type", is(beerDTO.getType().toString())));
    }
    ...
    void whenDELETEIsCalledWithValidIdThenNoContentStatusIsReturned() throws Exception {
        ...
        //when -> executamos a função
        doNothing().when(beerService).deleteById(beerDTO.getId());

        // then -> como é void, então isNoContent().
    ...
    void whenDELETEIsCalledWithInvalidIdThenNotFoundStatusIsReturned() throws Exception {
        //when -> não preciso do DTO, já que não irá retornar um. Lançaremos exceção
